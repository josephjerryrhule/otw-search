...
jQuery(document).ready(function ($) {
  ...
  // Function to perform search when user stops typing
  function doneTyping() {
    // Show loading indicator
    $(".otw-search-results-container").addClass("loading");

    var searchTerm = $(
      ".otw-search-input-container input[type='search']"
    ).val();
    $.ajax({
      url: ajax_data.ajax_url,
      type: "POST",
  ...
  function handleSearchInput() {
    ...
    if (searchTerm) {
      ...
      doneTyping(); // Perform search if search field is empty after a delay
    }
  }
